...
public class MissionImpl implements IMission {
    ...
    protected boolean generateThreats() {
        ...
        Log.w("generateThreats()", "Threat Level: " + missionPreferences.threatLevel + "; interal = " + internalThreats + ", external = " + externalThreats);
        ...
        Log.w("generateThreats()", "Normal Threats: " + normalThreats + "; Serious Threats: " + seriousThreats);
		...
		*/
        ...
        Log.d("generateThreats()", "Normal unconfirmed Threats: " + normalUnconfirmed + "; Serious unconfirmed Threats: " + seriousUnconfirmed);
        ...
        Log.d("generateThreats()", "Threats 1st phase: " + threatsFirstPhase + "; Threats 2nd phase: " + threatsSecondPhase);
        ...
        for (int i = 0; i < threatsSum; i++) {
            ...
            if (internalThreatsNumber > maxInternalThreatsNumber) {
                Log.i("generateThreats()", "Too many internal threats. Redoing.");
                return false;
            }
            ...
            do {
                ...
                if (newThreat.getThreatLevel() == Threat.THREAT_LEVEL_SERIOUS) {
                    if (newThreat.getThreatPosition() == Threat.THREAT_POSITION_EXTERNAL) {
                        if (phase < minTSeriousExternalThreat || phase > maxTSeriousExternalThreat)
                            continue;
                    } else {
                        if (phase < minTSeriousInternalThreat || phase > maxTSeriousInternalThreat)
                            continue;
                    }
                } else {
                    if (newThreat.getThreatPosition() == Threat.THREAT_POSITION_EXTERNAL) {
                        if (phase < minTNormalExternalThreat || phase > maxTNormalExternalThreat)
                            continue;
                    } else {
                        if (phase < minTNormalInternalThreat || phase > maxTNormalInternalThreat)
                            continue;
                    }
                }
                found = true;
            ...
            if (!found) {
                Log.i("generateThreats()", "Could not create mission due to phase restrictions. Redoing.");
                return false;
            }
        ...
        for (int i = 0; i < 8; i++) {
            if (threats[i] != null && threats[i].getThreatPosition() == Threat.THREAT_POSITION_EXTERNAL) {
                switch (generator.nextInt(3)) {
                    case 0:
                        if (lastSector != Threat.THREAT_SECTOR_BLUE)
                            threats[i].setSector(Threat.THREAT_SECTOR_BLUE);
                        else threats[i].setSector(Threat.THREAT_SECTOR_WHITE);
                        break;
                    case 1:
                        if (lastSector != Threat.THREAT_SECTOR_WHITE)
                            threats[i].setSector(Threat.THREAT_SECTOR_WHITE);
                        else threats[i].setSector(Threat.THREAT_SECTOR_RED);
                        break;
                    case 2:
                        if (lastSector != Threat.THREAT_SECTOR_RED)
                            threats[i].setSector(Threat.THREAT_SECTOR_RED);
                        else threats[i].setSector(Threat.THREAT_SECTOR_BLUE);
                        break;
                    default:
    ...
    protected boolean generateDataOperations() {
        ...
        for (int i = 0; i < 3; i++) {
            Log.v("generateThreats()", "Phase " + (i + 1) + ": Incoming Data = " + incomingData[i] + "; Data Transfers = " + dataTransfers[i]);
        }
    ...
    protected void generateTimes() {
        ...
        Log.v("generateTimes()", "White noise time: " + whiteNoiseTime);
    ...
    protected boolean generatePhases() {
        Log.i("generatePhases()", "Data gathered: Generating phases.");
        ...
        if (threats[3] != null && threats[3].getThreatLevel() == Threat.THREAT_LEVEL_NORMAL &&
            ...
            do {
                ...
                Log.i("generatePhases()", "Ambush in phase 1 at time: " + ambushTime);
        ...
        for (int i = 0; i <= last; i++) {
            ...
            // first event?
            if (first) {
                if (!eventList.addEvent(currentTime, threats[i]))
                    Log.w("generatePhases()", "Could not add first event to list (time " + currentTime + ") - arg!");
                first = false;
            } else {
                boolean done = false; // try until it fits
                int nextTime = 0;
        ...
        if (!ambushOccured && threats[7] != null && threats[7].getThreatLevel() == Threat.THREAT_LEVEL_NORMAL &&
            ...
            do {
                ...
                Log.i("generatePhases()", "Ambush in phase 2 at time: " + ambushTime);
        ...
        for (int i = 4; i <= last; i++) {
            ...
            if (first) {
                if (!eventList.addEvent(currentTime, threats[i]))
                    Log.w("generatePhases()", "Could not add first event to list in second phase (time " + currentTime + ") - arg!");
                first = false;
            } else {
                boolean done = false; // try until it fits
                int nextTime = 0;
